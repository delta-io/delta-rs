...
impl Action {
    ...
    pub fn from_parquet_record(
        ...
        Ok(match field.get_basic_info().name() {
            ...
            "commitInfo" => Action::commitInfo(Box::new(CommitInfo::from_parquet_record(col_data)?)),
            name => {
...
/// JobInfo contained in the CommitInfo
#[derive(Serialize, Deserialize, Debug, Default, Clone)]
#[serde(rename_all = "camelCase")]
pub struct JobInfo {
    /// The job identifier
...
/// NoteBookInfo contained in the CommitInfo
#[derive(Serialize, Deserialize, Debug, Default, Clone)]
#[serde(rename_all = "camelCase")]
...
/// CommitInfo holds provenance information about changes to the table. This action
/// is not stored in the checkpoint and has reduced compatibility guarantees.
/// Information stored in it is best effort.
#[derive(Serialize, Deserialize, Debug, Default, Clone)]
#[serde(rename_all = "camelCase")]
pub struct CommitInfo {
    ...
    pub operation_parameters: HashMap<String, Option<String>>,
    ...
    pub operation_metrics: Option<HashMap<String, Option<String>>>,
    /// User-defined commit metadata if it was specified
    pub user_metadata: Option<String>,
    /// Tags if it was specified
    pub tags: Option<HashMap<String, Option<String>>>,
}
...
impl CommitInfo {
    fn from_parquet_record(record: &parquet::record::Row) -> Result<Self, ActionError> {
        ...
        for (i, (name, _)) in record.get_column_iter().enumerate() {
            match name.as_str() {
                "version" => {
                    re.version = record.get_long(i).map(Some).unwrap_or(None);
                }
                "timestamp" => {
                    re.timestamp = record
                        .get_long(i)
                ...
                "user_id" => match record.get_string(i) {
                    Ok(user_id) => re.user_id = Some(user_id.clone()),
                    _ => re.user_id = None,
                },
                "user_name" => match record.get_string(i) {
                    Ok(user_name) => re.user_name = Some(user_name.clone()),
                    _ => re.user_name = None,
                },
                "operation" => {
                    re.operation = record
                ...
                "operation_parameters" => match record.get_map(0) {
                    Ok(operation_parameters_map) => {
                        ...
                        .map_err(|estr| {
                            ActionError::InvalidField(format!(
                                "Invalid operation_parameters for CommitInfo action: {}",
                                estr,
                            ))
                        })?;
                        re.operation_parameters = operation_parameters;
                    }
                    _ => {
                        re.operation_parameters = HashMap::new();
                    }
                },
                "job" => {
                    let job_record = record
                        .get_group(i)
                        .map_err(|_| gen_action_type_error("commitInfo", "job", "struct"))?;

                    let job_id = job_record
                        .get_string(0)
                        .map_err(|_| gen_action_type_error("commitInfo", "job.job_id", "string"))?
                        .clone();

                    let job_name = job_record
                        .get_string(0)
                        .map_err(|_| gen_action_type_error("commitInfo", "job.job_name", "string"))?
                        .clone();

                    let run_id = job_record
                        .get_string(0)
                        .map_err(|_| gen_action_type_error("commitInfo", "job.run_id", "string"))?
                        .clone();

                    let job_owner_id = job_record
                        .get_string(0)
                        .map_err(|_| {
                            gen_action_type_error("commitInfo", "job.job_owner_id", "string")
                        })?
                        .clone();

                    let trigger_type = job_record
                        .get_string(0)
                        .map_err(|_| {
                            gen_action_type_error("commitInfo", "job.trigger_type", "string")
                        })?
                        .clone();
                ...
                "notebook" => {
                    let notebook_record = record
                        .get_group(i)
                        .map_err(|_| gen_action_type_error("commitInfo", "notebook", "struct"))?;

                    let notebook_id = notebook_record
                        .get_string(0)
                        .map_err(|_| {
                            gen_action_type_error("commitInfo", "notebook.notebook_id", "string")
                        })?
                        .clone();

                    re.notebook = Some(NotebookInfo { notebook_id });
                }
                "cluster_id" => match record.get_string(i) {
                    Ok(cluster_id) => re.cluster_id = Some(cluster_id.clone()),
                    _ => re.cluster_id = None,
                },
                "read_version" => {
                    re.read_version = record.get_long(i).map(Some).unwrap_or(None);
                }
                "isolation_level" => match record.get_string(i) {
                    Ok(isolation_level) => re.isolation_level = Some(isolation_level.clone()),
                    _ => re.isolation_level = None,
                },
                "is_blind_append" => {
                    re.is_blind_append = record.get_bool(i).map(Some).unwrap_or(None);
                }
                "operation_metrics" => match record.get_map(0) {
                    Ok(operation_metrics_map) => {
                        ...
                        .map_err(|estr| {
                            ActionError::InvalidField(format!(
                                "Invalid operation_metrics for CommitInfo action: {}",
                                estr,
                            ))
                        })?;
                        re.operation_metrics = Some(operation_metrics);
                    }
                    _ => {
                        re.operation_metrics = Some(HashMap::new());
                    }
                },
                "user_metadata" => match record.get_string(i) {
                    Ok(user_metadata) => re.user_metadata = Some(user_metadata.clone()),
                    _ => re.user_metadata = None,
                },
                "tags" => match record.get_map(0) {
                    Ok(tags_map) => {
                        let mut tags = HashMap::new();
                        populate_hashmap_with_option_from_parquet_map(&mut tags, tags_map)
                            .map_err(|estr| {
                                ActionError::InvalidField(format!(
                                    "Invalid tags for CommitInfo action: {}",
                                    estr,
                                ))
                            })?;
                        re.tags = Some(tags);
                    ...
                    _ => {
                        re.tags = Some(HashMap::new());
                    }
                },
                _ => log::warn!(
                    "Unexpected field name `{}` for commitinfo action: {:?}",
                    name,
                    record
                ),
            }
