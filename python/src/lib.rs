...
use deltalake::kernel::transaction::{
    CommitBuilder, CommitProperties, ReservedCommitInfoKey, TableReference,
};
...
fn validate_reserved_commit_metadata(key: &str, value: &Value) -> PyResult<()> {
    let Some(reserved_key) = ReservedCommitInfoKey::from_name(key) else {
        return Ok(());
    };

    match reserved_key {
        ReservedCommitInfoKey::Timestamp
        | ReservedCommitInfoKey::Operation
        | ReservedCommitInfoKey::EngineInfo => Err(PyValueError::new_err(format!(
            "CommitProperties.custom_metadata key '{key}' is generated by delta-rs and cannot be set"
        ))),
        ReservedCommitInfoKey::OperationParameters => {
            if value.is_object() {
                Ok(())
            } else {
        ...
        ReservedCommitInfoKey::ReadVersion => {
        ...
        | ReservedCommitInfoKey::UserMetadata => {
            if value.is_string() {
                ...
                Err(PyValueError::new_err(format!(
                    "CommitProperties.custom_metadata['{key}'] must be a string"
                )))
            }
        }
        ReservedCommitInfoKey::IsolationLevel => {
            let valid = value
                .as_str()
        ...
        ReservedCommitInfoKey::IsBlindAppend => {
            if value.is_boolean() {
                Ok(())
            } else {
                Err(PyValueError::new_err(
        ...
        _ => Err(PyValueError::new_err(format!(
            "CommitProperties.custom_metadata key '{key}' is reserved by delta-rs and is not supported"
        ))),
    }
}
