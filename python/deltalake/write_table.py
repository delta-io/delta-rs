...
def write_deltalake_ray(
    ...
    *,
    ...
    writer_actor = WriterActor.remote()
    ...
    if table is not None:
        ...
        for pa_table_ref in data.to_arrow_refs():
            if len(result_batch_refs) > MAX_NUM_PENDING_TASKS:
                ...
                ready_refs, result_batch_refs = ray.wait(
                    result_batch_refs, num_returns=1
                )
                batch_references += ray.get(ready_refs)
            result_batch_refs.append(
                writer_actor.batch_validate.remote(
                    pa_table_ref, new_table_data, mode, partition_filters
                )
            )
        batch_references += ray.get(result_batch_refs)

        for val in batch_references:
            if val is False:
