...
{_DNF_filter_doc}
    ...
    async def load_with_datetime_async(self, datetime_string: str) -> None:
        """
        Asynchronous Time travel Delta table to the latest version that's created at or before provided `datetime_string` argument.
        The `datetime_string` argument should be an RFC 3339 and ISO 8601 date and time string.
        ...
        :param datetime_string: the identifier of the datetime point of the DeltaTable to load
        """
        await self._table.load_with_datetime_async(datetime_string)
    ...
    async def history_async(self, limit: Optional[int] = None) -> List[Dict[str, Any]]:
        """
        Asynchronously Run the history command on the DeltaTable.
        The operations are returned in reverse chronological order.
        ...
        return [
            json.loads(commit_info_raw)
            for commit_info_raw in await self._table.history_async(limit)
        ]
    ...
    async def update_incremental_async(self) -> None:
        """
        Asynchronously updates the DeltaTable to the latest version by incrementally applying
        newer versions.
        """
        await self._table.update_incremental_async()
