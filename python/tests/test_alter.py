...
def test_add_column_primitive(existing_table: DeltaTable):
    ...
    existing_table.alter.add_column(new_fields_to_add)
...
def test_add_timestamp_ntz_column(tmp_path: pathlib.Path, sample_table: pa.Table):
    ...
    new_fields_to_add = Field("timestamp_ntz_col", PrimitiveType("timestampNtz"))

    dt.alter.add_column(new_fields_to_add)
