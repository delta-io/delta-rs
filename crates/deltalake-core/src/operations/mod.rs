...
impl DeltaOps {
    ...
    pub fn create(self) -> CreateBuilder {
        CreateBuilder::default().with_log_store(self.0.log_store.clone())
    }
    ...
    pub fn load(self) -> LoadBuilder {
        LoadBuilder::new(self.0.log_store.clone(), self.0.state)
    }
    ...
    pub fn write(self, batches: impl IntoIterator<Item = RecordBatch>) -> WriteBuilder {
        WriteBuilder::new(self.0.log_store.clone(), self.0.state).with_input_batches(batches)
    }

    /// Vacuum stale files from delta table
    #[must_use]
    pub fn vacuum(self) -> VacuumBuilder {
        VacuumBuilder::new(self.0.log_store.clone(), self.0.state)
    }

    /// Audit active files with files present on the filesystem
    #[must_use]
    pub fn filesystem_check(self) -> FileSystemCheckBuilder {
        FileSystemCheckBuilder::new(self.0.log_store.clone(), self.0.state)
    }
    ...
    pub fn optimize<'a>(self) -> OptimizeBuilder<'a> {
        OptimizeBuilder::new(self.0.log_store.clone(), self.0.state)
    }
    ...
    pub fn delete(self) -> DeleteBuilder {
        DeleteBuilder::new(self.0.log_store.clone(), self.0.state)
    }
    ...
    pub fn update(self) -> UpdateBuilder {
        UpdateBuilder::new(self.0.log_store.clone(), self.0.state)
    }

    /// Restore delta table to a specified version or datetime
    #[must_use]
    pub fn restore(self) -> RestoreBuilder {
        RestoreBuilder::new(self.0.log_store.clone(), self.0.state)
    }
