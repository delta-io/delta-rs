    ...
    async fn write_commit_entry(
        ...
        match commit_or_bytes {
            ...
            CommitOrBytes::TmpCommit(tmp_commit) => {
                Ok(super::write_commit_entry(&self.object_store(), version, &tmp_commit).await?)
            } // This will only hit if we have an S3 Store which has copy-if-not-exists or unsafe-rename enbaled
        }
