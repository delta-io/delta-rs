...
/// A key that maps to a typed [`CommitInfo`] field rather than flattened custom metadata.
///
/// These fields are not required by the protocol, but leaving them in flattened metadata can
/// produce duplicate JSON keys. Bindings should use [`Self::from_name`] to share this contract
/// with core transaction normalization.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
#[non_exhaustive]
pub enum ReservedCommitInfoKey {
    /// Commit timestamp generated by delta-rs.
    Timestamp,
    /// Identifier for the user performing the commit.
    UserId,
    /// Name of the user performing the commit.
    UserName,
    /// Operation name generated by delta-rs.
    Operation,
    /// Parameters associated with the operation.
    OperationParameters,
    /// Table version read by the operation.
    ReadVersion,
    /// Isolation level used by the operation.
    IsolationLevel,
    /// Whether the operation appended data without reading the table.
    IsBlindAppend,
    /// Engine information generated by delta-rs.
    EngineInfo,
    /// User-provided commit metadata.
    UserMetadata,
}

impl ReservedCommitInfoKey {
    /// Resolve a serialized `commitInfo` field name to its reserved key.
    pub fn from_name(name: &str) -> Option<Self> {
        match name {
            ...
            "operationParameters" => Some(Self::OperationParameters),
            "readVersion" => Some(Self::ReadVersion),
            "isolationLevel" => Some(Self::IsolationLevel),
            "isBlindAppend" => Some(Self::IsBlindAppend),
            "engineInfo" => Some(Self::EngineInfo),
            "userMetadata" => Some(Self::UserMetadata),
            _ => None,
        }
    }

    /// Return the serialized `commitInfo` field name.
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::IsBlindAppend => "isBlindAppend",
...
/// existing tolerant behavior: invalid reserved metadata is logged and dropped. Bindings can
/// classify the same key set through [`ReservedCommitInfoKey::from_name`] and reject invalid input
...
fn normalize_reserved_commit_metadata(
    ...
    match app_metadata.remove(ReservedCommitInfoKey::OperationParameters.as_str()) {
        ...
        Some(value) => log_unexpected_reserved_metadata_type(
            ReservedCommitInfoKey::OperationParameters.as_str(),
            &value,
    ...
    if let Some(value) = app_metadata.remove(ReservedCommitInfoKey::ReadVersion.as_str()) {
        if let Some(value) = value.as_u64() {
            ...
            log_unexpected_reserved_metadata_type(
                ReservedCommitInfoKey::ReadVersion.as_str(),
                &value,
    ...
    promote_string_reserved_metadata(
        app_metadata,
        ReservedCommitInfoKey::UserId,
        &mut commit_info.user_id,
    );
    promote_string_reserved_metadata(
        app_metadata,
        ReservedCommitInfoKey::UserName,
        &mut commit_info.user_name,
    );
    promote_string_reserved_metadata(
        app_metadata,
        ReservedCommitInfoKey::UserMetadata,
        &mut commit_info.user_metadata,
    );

    if let Some(value) = app_metadata.remove(ReservedCommitInfoKey::IsolationLevel.as_str()) {
            ...
            .and_then(|value| IsolationLevel::from_str(value).ok())
        {
            ...
            log_unexpected_reserved_metadata_type(
                ReservedCommitInfoKey::IsolationLevel.as_str(),
                &value,
    ...
    if let Some(value) = app_metadata.remove(ReservedCommitInfoKey::IsBlindAppend.as_str()) {
        if let Some(value) = value.as_bool() {
            ...
            log_unexpected_reserved_metadata_type(
                ReservedCommitInfoKey::IsBlindAppend.as_str(),
                &value,
                "boolean",
            );
        }
    ...
    app_metadata.retain(|key, _| ReservedCommitInfoKey::from_name(key).is_none());
}

fn promote_string_reserved_metadata(
    metadata: &mut HashMap<String, Value>,
    key: ReservedCommitInfoKey,
...
fn assign_commit_info_metadata(
    ...
    debug_assert!(
        app_metadata
            .keys()
            .all(|key| ReservedCommitInfoKey::from_name(key).is_none())
    );
...
mod tests {
    ...
    #[test]
    fn test_reserved_commit_info_key_names_round_trip() {
        for name in [
            "timestamp",
            "userId",
            "userName",
            ...
            let key = ReservedCommitInfoKey::from_name(name)
                .unwrap_or_else(|| panic!("missing reserved commit info key {name}"));
            assert_eq!(key.as_str(), name);
        }
        assert_eq!(ReservedCommitInfoKey::from_name("clientVersion"), None);
        assert_eq!(ReservedCommitInfoKey::from_name("custom"), None);
    }
    ...
    fn test_commit_data_strips_and_promotes_reserved_metadata() {
        ...
        for key in info.info.keys() {
            assert!(
                ReservedCommitInfoKey::from_name(key).is_none(),
