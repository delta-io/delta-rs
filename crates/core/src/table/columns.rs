...
/// An identity column definition extracted from field metadata
#[derive(Eq, PartialEq, Debug, Clone)]
pub struct IdentityColumnInfo {
    ...
    pub start: i64,
    /// Increment step (cannot be 0)
    pub step: i64,
    /// The highest (positive step) or lowest (negative step) value generated so far
    pub high_water_mark: Option<i64>,
    /// If true: GENERATED BY DEFAULT (user values allowed)
    /// If false: GENERATED ALWAYS (user values blocked)
