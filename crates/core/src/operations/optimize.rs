                ...
                .map(|(partition, files)| {
                    ...
                    let batch_stream = futures::stream::iter(files.clone())
                        .then(move |file| {
                            ...
                            async move {
                                let file_reader = ParquetObjectReader::new(object_store_ref, ObjectMeta::try_from(file).unwrap());
            ...
            OptimizeOperations::ZOrder(zorder_columns, bins) => {
                ...
                futures::stream::iter(bins)
                    .map(move |(_, (partition, files))| {
                        let batch_stream = Self::read_zorder(files.clone(), exec_context.clone(), DeltaTableProvider::try_new(snapshot.clone(), log_store.clone(), scan_config.clone()).unwrap());
                        let rewrite_result = tokio::task::spawn(Self::rewrite_files(
...
fn build_compaction_plan(
    ...
    let mut partition_files: HashMap<String, (IndexMap<String, Scalar>, Vec<Add>)> =
        HashMap::new();
