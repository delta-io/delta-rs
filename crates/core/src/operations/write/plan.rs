...
use delta_kernel::{
    engine::arrow_conversion::TryIntoArrow as _, expressions::ColumnName,
    table_configuration::TableConfiguration, table_features::TableFeature,
};
...
pub(crate) fn plan_insert(
    ...
    let gen_lookup = if table_config.is_feature_enabled(&TableFeature::GeneratedColumns) {
    ...
    let (plan, action) = if missing_in_source > 0 && missing_in_table > 0 {
        ...
        if !matches!(schema_mode, Some(SchemaMode::Merge)) {
            return plan_err!(
                "Schema evolution required but schema mode not set to Merge. \
                Please set schema mode to Merge to enable schema evolution."
            );
        }
        ...
        .to_dfschema()?;
        let projection = project_for_insert(&merged_schema, &source_schema, gen_lookup, safe_cast)?;
