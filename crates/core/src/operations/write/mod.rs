...
impl WriteBuilder {
    ...
    /// Specify the save mode for the write operation
    ///
    /// - `Append`: Add the new data to the existing data
    /// - `Overwrite`: Replace the existing data with the new data
    ///   (optionally with a predicate)
    /// - `ErrorIfExists`: Raise an error if data already exists
    ...
    /// Set the schema mode for handling schema drift/evolution
    ///
    /// - `Overwrite`: Overwrite the schema with the schema of the input data
    /// - `Merge`: Merge the schema of the input data with the existing schema
    ...
    /// Specify table partitioning columns.
    ///
    /// This is nneds to be specified when creating a new table.
    ///
    /// If the table already exists, the provided partition columns are validated
    /// against the existing table partitioning. An error is returned id the partitions
    /// do not match. Otherwise, the provided partition columns are used to partition
    /// the new table.
    ...
    /// Specify the [`Session`] to use when executing the input plan.
    ///
    /// If not provided, a default session will be created. When providing
    /// a custom session, ensure that it is configured with the necessary
    /// extensions and options to read and write Delta tables.
    ///
    /// You can use the [`create_session`] function to create a session with
    /// Delta extensions and further details on how to customize the session.
    ...
    #[deprecated(
        since = "0.31.0",
        note = "Convert input batches to a logical plan using `with_input_plan` instead"
    )]
    ...
    fn into_future(self) -> Self::IntoFuture {
        ...
        Box::pin(
            async move {
                ...
                // but we will have the the configuration the table will be created with.
                ...
                let table_config = if let Some(snapshot) = &maybe_snapshot {
                    if this.mode == SaveMode::ErrorIfExists {
                        return Err(DeltaTableError::generic(
                            "Table already exists and SaveMode is ErrorIfExists",
                        ));
                    } else if this.mode == SaveMode::Ignore {
                        return Err(DeltaTableError::generic(
                            "Save mode is Ignore is not supported for for write operations.",
                        ));
                    }
...
/// Plan and execute the write operation
///
/// During execution we plan the necessary steps to insert data into the table,
/// including any schema changes that may be required. If we are replacing data
/// based on a predicate, we also plan the necessary deletions and re-insertions.
///
/// If CDC is enabled on the table, we transform the data into a CDC change set
/// including the appropriately labeled inserts and deletes.
async fn execute(
    session: &dyn Session,
    ...
    // We need to validate data against the table constraints. The validation
    // will also handle updating the nullability of columns after proving that
    // the data conforms to the constraints defined in the table sdchema.
    let predicates = validation_predicates(session, source.schema().as_ref(), table_config)?;
    if !predicates.is_empty() {
        source = source.into_builder().validate(predicates)?.build()?;
    }

    // We apply the predicate against several sub-tables so we need to avoid qualified
    // column names coupling with somewhat arbitrary internal naming. So we strip qualifiers here.
    let stripped_schema = Arc::new(source.schema().as_ref().clone().strip_qualifiers());
    let predicate = maybe_predicate
        .map(|p| p.resolve(session, stripped_schema))
        .transpose()?;
    ...
    mod check_preconditions_test {
        ...
        async fn test_no_snapshot_create_actions() -> DeltaResult<()> {
            ...
            let table = DeltaTable::new_in_memory()
                ...
                .await?;
            let batch = get_record_batch(None, false);
            let plan = LogicalPlanBuilder::scan_batches("scan", batch.schema(), vec![Some(batch)])?
                .build()?;
            let mut writer = WriteBuilder::new(table.log_store.clone(), None).with_input_plan(plan);
        ...
        async fn test_no_snapshot_err_no_batches_check() -> DeltaResult<()> {
            ...
            let table = DeltaTable::new_in_memory()
                ...
                .with_columns(table_schema.fields().cloned())
                .await?;
            let mut writer = WriteBuilder::new(table.log_store.clone(), None)
                .with_input_plan(LogicalPlanBuilder::empty(false).build().unwrap());
