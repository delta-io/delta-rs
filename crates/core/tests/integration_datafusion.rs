...
mod local {
    ...
    async fn test_datafusion_simple_query_partitioned() -> Result<()> {
        ...
        let table =
            open_table(ensure_table_uri("../test/tests/data/delta-0.8.0-partitioned").unwrap())
                .await
                .unwrap();
    ...
    async fn test_datafusion_optimize_stats_partitioned_pushdown() -> Result<()> {
        ...
        let table =
            open_table(ensure_table_uri("../test/tests/data/http_requests").unwrap()).await?;
    ...
    async fn test_files_scanned_pushdown_limit() -> Result<()> {
        ...
        let ctx = SessionContext::new();
        let state = ctx.state();
        let table = open_table(ensure_table_uri("../test/tests/data/delta-0.8.0").unwrap()).await?;
    ...
    async fn test_datafusion_write_from_serialized_delta_scan() -> Result<()> {
        ...
        let source_scan_bytes = {
            let source_table =
                open_table(ensure_table_uri("../test/tests/data/delta-0.8.0-date").unwrap())
                    .await?;
    ...
    async fn test_datafusion_date_column() -> Result<()> {
        let ctx = SessionContext::new();
        let table = open_table(ensure_table_uri("../test/tests/data/delta-0.8.0-date").unwrap())
            .await
            .unwrap();
    ...
    async fn test_datafusion_stats() -> Result<()> {
        ...
        let table = open_table(ensure_table_uri("../test/tests/data/delta-0.8.0").unwrap())
            .await
            .unwrap();
        ...
        let table = open_table(ensure_table_uri("../test/tests/data/delta-0.2.0").unwrap())
            .await
            .unwrap();
        let statistics = table.snapshot()?.datafusion_table_statistics().unwrap();
    ...
    async fn test_datafusion_partitioned_types() -> Result<()> {
        ...
        let table = open_table(
            ensure_table_uri("../test/tests/data/delta-2.2.0-partitioned-types").unwrap(),
        )
        .await
        .unwrap();
        ctx.register_table("demo", Arc::new(table))?;
    ...
    async fn test_datafusion_scan_timestamps() -> Result<()> {
        let ctx = SessionContext::new();
        let table =
            open_table(ensure_table_uri("../test/tests/data/table_with_edge_timestamps").unwrap())
                .await
                .unwrap();
        ctx.register_table("demo", Arc::new(table))?;
    ...
    async fn test_issue_1374() -> Result<()> {
        let ctx = SessionContext::new();
        let table = open_table(ensure_table_uri("../test/tests/data/issue_1374").unwrap())
            .await
            .unwrap();
    ...
    async fn test_issue_1619_parquet_panic_using_map_type() -> Result<()> {
        ...
        let table = deltalake_core::DeltaTableBuilder::from_uri(
            ensure_table_uri("./tests/data/issue-1619").unwrap(),
        )?
        .build()?;
        ...
        let mut table = open_table(ensure_table_uri("./tests/data/issue-1619").unwrap()).await?;
